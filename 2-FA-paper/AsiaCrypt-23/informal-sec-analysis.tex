...
\begin{itemize}
\item[$\bullet$]\textit{T.DEV: token access.} An adversary may steal the authentication token. The adversary will then know \VC{k}, \VC{\salt} and the current values of \VC{\counter} and \VC{\state} because we assume the token does not take advantage of a trusted chipset. 
