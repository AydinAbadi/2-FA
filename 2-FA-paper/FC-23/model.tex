...
Figure \ref{fig:setup.} outlines the workflow of the 2FA scheme during the authentication phase.  At a very high level, the authentication phase works as follows. Any time the user wants to authenticate itself to the server, it sends a message to it via the client.  The server replies to the client with a challenge and transaction details (Step 1). The user uses the device to scan the message that the client received (Step 2). The device shows the transaction details on the screen (Step 3). The user inserts the PIN into the device (Step 4). The device generates a verifier (Step 5). The user manually inserts the verifier into the client (Step 6).  The client sends the verifier to the server for authentication (Step 7). 
