...
\begin{center}
    \begin{tcolorbox}[enhanced,width=4.7in, left=.1cm,
...
{\small{
 ...
 \doubleunderline{\execute($C^{\sss i}, S^{\sss j}$)}\\
 
 This query is dealt with as below: 
 %
 \begin{enumerate} 
 %
 \item $(C_{\sss ID},  \text{enrolment})\gets$\send($C^{\sss i}, \text{start}, \text{enrolment}$).%, where $\text{phase}\in \{\text {enrollment, authentication}\}$. 
 %
